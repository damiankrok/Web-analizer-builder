...
describe('holdout isolation (§28)', () => {
  it('registers exactly one holdout, and it is not A, B or the historical C', () => {
    const holdouts = PROJECTS.filter((p) => p.role === 'HOLDOUT')
    expect(holdouts).toHaveLength(1)
    expect(holdouts[0].key).toBe('D')
    const historical = PROJECTS.find((p) => p.key === 'C')
    expect(historical?.role).toBe('HISTORICAL_HOLDOUT_WEB01')
  })
